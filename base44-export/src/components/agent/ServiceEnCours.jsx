...
export default function ServiceEnCours({
  ...
  const prolongMut = useMutation({
    mutationFn: async () => {
      const text = motif.trim();
      if (text.length < 3) throw new Error('Indiquez le motif de la prolongation.');
      const now = format(new Date(), 'HH:mm');
      await base44.entities.PriseDeService.update(service.id, {
        prolongation_motif: text,
        prolongation_at: new Date().toISOString(),
        prolongation_time: now,
      });
      await writeMc('prolongation_service', `Prolongation de service à ${now} — motif : ${text}`, { severity: 'attention' });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['prises_service'] });
      toast.success('Prolongation enregistrée');
      setMotif('');
    },
    onError: (e) => toast.error(e.message || 'Prolongation impossible'),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      {needsProlongation && (
        <Card className="p-4 border-amber-400 bg-amber-50 space-y-3">
          <h3 className="font-semibold">Prolongation de service</h3>
          <p className="text-sm text-amber-950">
            L’heure de fin ({service.planned_end}) est dépassée. Pour continuer, déclarez une prolongation et indiquez le motif. Terminer le service reste possible ; vous ne pourrez plus le reprendre ensuite.
          </p>
          <Textarea
            rows={3}
            value={motif}
            onChange={(e) => setMotif(e.target.value)}
            placeholder="Motif obligatoire (client encore présent, relève en retard…)"
          />
          <Button
            className="w-full h-12 bg-amber-500 hover:bg-amber-600 text-amber-950"
            disabled={motif.trim().length < 3 || prolongMut.isPending}
            onClick={() => prolongMut.mutate()}
          >
      ...
      <Card className="p-4">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            disabled={onPause || needsProlongation}
          >
          ...
          <div className="mt-3 space-y-2">
            {siteRondes.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-2 text-sm">
                <span>{r.name}</span>
                <Button size="sm" variant="secondary" disabled={onPause || needsProlongation} onClick={() => onStartRonde(r)}>Démarrer</Button>
              </div>
            ))}
          </div>
        )}
      ...
      <Card className="p-4">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            disabled={pauseMut.isPending || needsProlongation}
          >
